...
2. The columns are interpolated to polynomials of degree $<n$ in the univariate case and as multilinear polynomials in the multivariate case.
...
A set of endomorphisms of $\mathbb{F}^v$ (which are actually permutations) satifying this condition is given by compositions of bit-flips and coordinate permutations. Formally, they are given by
$$
...
> For every $\sigma \in L_v$ and $C\in \mathbb{F}[X_1,\dots,X_{2w}]$, there is an efficient proving system based on the multivariate sumcheck protocol for the EAR on $n = 2^v$ rows, $w$ columns, and constraint $C$. 
...
This suggests the following construction. Let $\sigma$ be a permutation with largest cycle of length $\mu = M(v)$ and $\nu = N(v)$ such cycles with supports $\Pi_1,\dots,\Pi_\nu \subset \\{0,1\\}^v$. 

Then, the EAR with endomorphism $\sigma$ and constraint $C$ contains $\nu$ copies of an AIR with $\mu$ rows and constraint $C$. It can thus be used to compute $\nu$ AIR on the row sets $\Pi_1,\dots,\Pi_\nu$ in parallel. Note that this construction also works more generally with RAPs.
...
- One might want to use different constraints $C_1,\dots,C_\nu$ on the different row sets $\Pi_1,\dots,\Pi_\nu$. This can also be solved by adding preprocessed polynomials. Either $\nu$ selector polynomials taking binary values or one polynomial taking values $1$ to $\nu$.
